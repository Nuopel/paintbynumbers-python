...
class PaintByNumbersPipeline:
    ...
    def process(
        ...
        # Stage 4-7: Iterative refinement (cleanup, build, reduce)
        # Must run in a loop so facets are rebuilt after each color map update
        num_iterations = max(1, settings.narrowPixelStripCleanupRuns)
        facet_result = None

        for run in range(num_iterations):
            # Stage 4: Clean narrow strips (only if enabled)
            if settings.narrowPixelStripCleanupRuns > 0:
                iteration_progress = run / num_iterations
                update("Cleaning narrow strips", iteration_progress)
                ColorReducer.process_narrow_pixel_strip_cleanup(color_map_result)

            # Stage 5: Build facets with updated color map
            iteration_progress = run / num_iterations
            update("Building facets", iteration_progress)
            ...
            update("Building neighbors", iteration_progress)
            for i, facet in enumerate(facet_result.facets):
                if facet is not None:
                    facet_builder.build_facet_neighbour(facet, facet_result)

            # Stage 7: Reduce facets (only if enabled)
            # This updates color indices for the next iteration
            if settings.removeFacetsSmallerThanNrOfPoints > 0 or (settings.maximumNumberOfFacets is not None and settings.maximumNumberOfFacets < len(facet_result.facets)):
                FacetReducer.reduce_facets(
                    ...
                    facet_result,
                    color_map_result.imgColorIndices,
                    on_update=lambda p: update("Reducing facets", iteration_progress + (p / num_iterations))
                )

        # Mark all stages complete
        update("Cleaning narrow strips", 1.0)
